...
import { setCurrentProject } from "@/shared/store/slices/project.slice";
    ...
    return (
        <div className="flex flex-col gap-y-3.5 rounded-lg p-6 bg-white h-full relative">
            <div className="flex justify-between items-start w-full">
                <Link
                    to={`/project/${project.id}/epics`}
                    onClick={handleSelectProject}
                    className="text-title-md text-slate-dark capitalize pr-4"
                >
                    {project?.name}
                </Link>
                <Link
            ...
            <Link
                to={`/project/${project.id}/epics`}
                onClick={handleSelectProject}
                className="flex flex-col gap-y-3.5 h-full justify-between"
            >
